...
export const LevelUpRewardCreate = (props: {
  guildID: string;
  onCancel: () => void;
  onCreate: (reward: LevelUpRewardType) => void;
}) => {
  const { onCancel, onCreate, guildID } = props;
  const [reward, setReward] = useState({
    type: "atLevel",
    guildID: guildID,
    level: 1,
    name: "New Level Up Reward",
    rewards: [],
    _id: "",
    ...
    >
      <div className={`flex flex-col gap-2 w-full`}>
        ...
        <div className={`flex flex-row gap-2 items-center`}>
            ...
            onClick={async () => {
              ...
              const res = await fetcher(
                `${await getGuildShardURL(
                  guildID
                )}/guilds/${guildID}/settings/levelrewards/`,
                {
                  method: "POST",
          ...
          >
            <span className={`font-wsans`}>Discard</span>
          </div>
        </div>
      </div>
    </div>
